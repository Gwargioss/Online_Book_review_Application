...
describe("review routes", () => {
  ...
  it("PUT /api/v1/books/:id/reviews", async () => {
    reviewService.upsertReview.mockResolvedValue({ saved: true });
    const app = await makeApp();
    ...
    const res = await request(app)
      ...
      .send({ review_text: "Excellent!" });

    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  });

  it("DELETE /api/v1/books/:id/reviews", async () => {
    reviewService.deleteReview.mockResolvedValue({ deleted: true });
    const app = await makeApp();
    const token = await makeToken();

    const res = await request(app)
      .delete("/api/v1/books/507f1f77bcf86cd799439011/reviews")
      .set("Authorization", `Bearer ${token}`);
